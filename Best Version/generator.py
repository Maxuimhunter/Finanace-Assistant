...
def generate_pdf(insights_text, excel_data_str=None):
    ...
    for line in str(insights_text).split('\n'):
        ...
        if line.startswith(('*', '-')):
            ...
            pdf.cell(10, 10, '*', 0, 0)
            pdf.set_font('Arial', '', 12)
            # Handle long lines by splitting them into smaller chunks
            text = line[1:].strip()
            if pdf.get_string_width(text) > pdf.w - 30:  # Account for margins
                pdf.set_font('Arial', '', 10)  # Smaller font for long lines
            pdf.multi_cell(0, 10, text)
            if pdf.get_string_width(text) > pdf.w - 30:
        ...
        # Handle section headers
            ...
            # Handle long lines by splitting them into smaller chunks
            if pdf.get_string_width(line) > pdf.w - 30:  # Account for margins
                pdf.set_font('Arial', '', 10)  # Smaller font for long lines
            pdf.multi_cell(0, 10, line)
            if pdf.get_string_width(line) > pdf.w - 30:
                pdf.set_font('Arial', '', 12)  # Reset font
