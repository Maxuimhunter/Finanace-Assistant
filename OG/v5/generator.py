...
def main():
    ...
    if page == "Home":
        ...
        st.caption("This is how your Excel file will be organized:")
        ...
        financial_sections = []
        if income: financial_sections.append("Income")
        if expenses: financial_sections.append("Expenses")
        if savings: financial_sections.append("Savings")
        if stocks: financial_sections.append("Stocks")
        if financial_sections:
            selected_sections.append(("Financial", financial_sections))
        
        # Add Health sections
        health_sections = []
        if weight: health_sections.append("Weight")
        if habits: health_sections.append("Habits")
        if health_sections:
            selected_sections.append(("Health", health_sections))
        
        # Add Life Organization sections
        life_sections = []
        if cleaning: life_sections.append("Cleaning")
        if meals: life_sections.append("Meal Planning")
        if timetable: life_sections.append("Time Table")
        if life_sections:
            selected_sections.append(("Life Organization", life_sections))
        
        # Display the preview
        if selected_sections:
            for section_name, items in selected_sections:
                with st.expander(f"{section_icons.get(section_name, '📁')} {section_name}"):
                    for item in items:
                        st.markdown(f"- {item}")
        else:
            st.info("No sections selected. The generated file will only include the Welcome Guide and AI Insights.")
