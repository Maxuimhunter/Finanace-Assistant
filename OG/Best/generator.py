...
def generate_pdf(insights_text, excel_data_str=None):
    ...
    # Use standard fonts that come with fpdf2
    default_font = 'Arial'
    
    # Set margins (left, top, right)
    pdf.set_margins(15, 15, 15)
    
    # Auto-page-break with 15mm margin from bottom
    ...
    # Set up the document with standard font
    ...
    pdf.cell(0, 8, f'Generated on: {datetime.datetime.now().strftime("%B %d, %Y at %I:%M %p")}', 0, 1, 'C')
    pdf.ln(8)
    ...
    # Add insights header with subtle background
    pdf.set_fill_color(240, 244, 255)  # Very light blue
    pdf.set_font(default_font, 'B', 14)
    pdf.cell(0, 10, 'KEY FINANCIAL INSIGHTS', 0, 1, 'C', 1)
    pdf.ln(5)
    
    # Helper function to safely add text with proper wrapping
    def add_wrapped_text(text, font_style='', font_size=12, indent=0):
        # Set the font
        ...
        pdf.set_font(default_font, font_style, font_size)
        ...
        for line in text.split('\n'):
            ...
            # Split long lines into chunks that fit the page width
            max_width = pdf.w - pdf.l_margin - pdf.r_margin - (indent * 2)
            
            # If the line is too long, split it into words and build lines
    ...
    # Process each line of the insights
    current_section = []
    ...
    for line in str(insights_text).split('\n'):
        ...
        if not line:
            if current_section:
                # Add the accumulated section
                add_wrapped_text(' '.join(current_section))
                current_section = []
            pdf.ln(5)
            continue
        ...
        if line.startswith(('*', '-')):
            if current_section:
                add_wrapped_text(' '.join(current_section))
                current_section = []
            add_wrapped_text(line[1:].strip(), 'B', 12, 10)
        # Handle section headers
        elif ':' in line and len(line) < 50:
            if current_section:
                add_wrapped_text(' '.join(current_section))
                current_section = []
            add_wrapped_text(line, 'B', 12)
        else:
    ...
    if excel_data_str:
        ...
        pdf.set_font(default_font, 'B', 14)
        pdf.cell(0, 10, 'DETAILED FINANCIAL DATA', 0, 1, 'C', 1)
        pdf.ln(8)
        
        # Add a note about graphs (placeholder for future implementation)
        pdf.set_font(default_font, 'I', 10)
        pdf.multi_cell(0, 6, "Note: Graphical representations of this data will be included in future updates to provide better visualization of financial trends.", 0, 'C')
        pdf.ln(10)
        
        # Add the data in a clean, monospaced font
        pdf.set_font('Courier', '', 9)
        pdf.multi_cell(0, 5, clean_text(str(excel_data_str)))
