...
def parse_monzo_pdf_statement(file_content):
    """Parse Monzo PDF statement by extracting text"""
    try:
        import PyPDF2
        import io
        import re
        ...
        # Parse Monzo PDF format - ultra-comprehensive approach to capture ALL transactions
        all_transactions = []
        lines = text.split('\n')
            ...
            if amount_matches:
                for amount_str in amount_matches:
                    try:
                        ...
                        # Extract description (everything before this amount)
                        ...
                        # Find date in this line or nearby lines
        ...
        for trans in all_transactions:
            ...
            # Skip obvious invalid transactions
            if (len(desc) < 2 or 
                desc.isdigit() or
                any(keyword in desc_lower for keyword in [
                    'balance', 'total', 'account', 'pot', 'statement', 'anthony', 'kinyua', 'gathukia',
                    'flat', 'room', 'house', 'court', 'road', 'nottingham', 'ng7', 'united', 'kingdom'
                ])):
                continue
            
            # Create deduplication key
            key = (trans['Date'], desc_lower, trans['Amount'])
            if key not in seen:
                seen.add(key)
        ...
        if unique_transactions:
            ...
            print(f"Successfully parsed {len(unique_transactions)} transactions from PDF (from {len(all_transactions)} raw matches)")
...
def categorize_transaction(description):
    ...
    # Income categories
    if any(keyword in description for keyword in ['salary', 'wages', 'pay', 'income', 'deposit']):
        return 'Income'
    elif any(keyword in description for keyword in ['refund', 'return']):
        return 'Refund'
    
    # Expense categories
    elif any(keyword in description for keyword in ['rent', 'mortgage', 'property']):
        return 'Housing'
    elif any(keyword in description for keyword in ['tesco', 'sainsbury', 'asda', 'grocery', 'food', 'supermarket']):
        return 'Food & Groceries'
    elif any(keyword in description for keyword in ['uber', 'taxi', 'bus', 'train', 'tube', 'transport']):
        return 'Transportation'
    elif any(keyword in description for keyword in ['electric', 'gas', 'water', 'bill', 'utility']):
        return 'Utilities'
    elif any(keyword in description for keyword in ['netflix', 'spotify', 'subscription', 'prime']):
        return 'Subscriptions'
    elif any(keyword in description for keyword in ['restaurant', 'cafe', 'coffee', 'dining', 'eat']):
        return 'Dining Out'
    elif any(keyword in description for keyword in ['amazon', 'shop', 'store', 'purchase']):
        return 'Shopping'
    elif any(keyword in description for keyword in ['gym', 'fitness', 'health']):
        return 'Health & Fitness'
    elif any(keyword in description for keyword in ['cinema', 'movie', 'entertainment', 'theatre']):
...
def analyze_financial_performance(df):
    ...
    try:
        ...
        # Category breakdown (top 5 categories only)
        expense_by_category = df[df['Amount'] < 0].groupby('Category')['Amount'].sum().abs().nlargest(5)
        
        # Recent transactions (top 5 only)
        recent_transactions = df.head(5).to_dict('records')
        ...
        {str(recent_transactions[:3])}
        
        Provide brief analysis covering:
        1. Financial health (1-2 sentences)
        2. Main spending patterns (1-2 sentences)
        3. Top savings opportunities (1-2 sentences)
        ...
        try:
            ...
            return "⚠️ Analysis timed out. Please try again or use a smaller statement file."
        except Exception as e:
